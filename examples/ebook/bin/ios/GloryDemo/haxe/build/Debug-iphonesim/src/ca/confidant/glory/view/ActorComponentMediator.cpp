...
{
HX_STACK_PUSH("ActorComponentMediator::new","ca/confidant/glory/view/ActorComponentMediator.hx",26);
{
	HX_STACK_LINE(27)
...
::ca::confidant::glory::view::components::ActorComponent ActorComponentMediator_obj::actor( ){
	HX_STACK_PUSH("ActorComponentMediator::actor","ca/confidant/glory/view/ActorComponentMediator.hx",131);
	HX_STACK_THIS(this);
	HX_STACK_LINE(131)
...
{
		HX_STACK_PUSH("ActorComponentMediator::onPanRight","ca/confidant/glory/view/ActorComponentMediator.hx",126);
		HX_STACK_THIS(this);
		HX_STACK_ARG(e,"e");
		HX_STACK_LINE(126)
		this->actor()->set_x((this->actor()->get_x() - (int)4));
	}
return null();
}
...
{
		HX_STACK_PUSH("ActorComponentMediator::onPanLeft","ca/confidant/glory/view/ActorComponentMediator.hx",120);
		HX_STACK_THIS(this);
		HX_STACK_ARG(e,"e");
		HX_STACK_LINE(120)
		this->actor()->set_x((this->actor()->get_x() + (int)4));
...
{
		HX_STACK_PUSH("ActorComponentMediator::handleNotification","ca/confidant/glory/view/ActorComponentMediator.hx",103);
		HX_STACK_THIS(this);
		...
		if (  ( _switch_1==HX_CSTRING("timerTick"))){
			...
			HX_STACK_LINE(108)
			this->actor()->addEventListener(::native::events::Event_obj::ENTER_FRAME,this->onPanLeft_dyn(),null(),null(),null());
		}
		else if (  ( _switch_1==HX_CSTRING("panRight"))){
			HX_STACK_LINE(110)
			this->actor()->addEventListener(::native::events::Event_obj::ENTER_FRAME,this->onPanRight_dyn(),null(),null(),null());
		}
		else if (  ( _switch_1==HX_CSTRING("panStop"))){
			HX_STACK_LINE(113)
			this->actor()->removeEventListener(::native::events::Event_obj::ENTER_FRAME,this->onPanRight_dyn(),null());
			HX_STACK_LINE(114)
			this->actor()->removeEventListener(::native::events::Event_obj::ENTER_FRAME,this->onPanLeft_dyn(),null());
		}
	}
return null();
}
...
Array< ::String > ActorComponentMediator_obj::listNotificationInterests( ){
	HX_STACK_PUSH("ActorComponentMediator::listNotificationInterests","ca/confidant/glory/view/ActorComponentMediator.hx",93);
	HX_STACK_THIS(this);
	HX_STACK_LINE(93)
...
Void ActorComponentMediator_obj::onActorClicked( ::native::events::MouseEvent e){
{
		HX_STACK_PUSH("ActorComponentMediator::onActorClicked","ca/confidant/glory/view/ActorComponentMediator.hx",46);
		HX_STACK_THIS(this);
		HX_STACK_ARG(e,"e");
		HX_STACK_LINE(47)
		Dynamic a = e->get_currentTarget();		HX_STACK_VAR(a,"a");
		HX_STACK_LINE(48)
		::haxe::Log_obj::trace(a->__Field(HX_CSTRING("name"),true),hx::SourceInfo(HX_CSTRING("ActorComponentMediator.hx"),48,HX_CSTRING("ca.confidant.glory.view.ActorComponentMediator"),HX_CSTRING("onActorClicked")));
		HX_STACK_LINE(51)
		::String _switch_2 = (this->config->type);
		if (  ( _switch_2==HX_CSTRING("control"))){
			HX_STACK_LINE(60)
			...
			HX_STACK_LINE(62)
			Array< ::String > actionsArray = this->config->action.split(HX_CSTRING(","));		HX_STACK_VAR(actionsArray,"actionsArray");
			HX_STACK_LINE(64)
			{
				HX_STACK_LINE(64)
				...
				while(((_g < actionsArray->length))){
					...
					else if (  ( _switch_3==HX_CSTRING("pageSkipTo"))){
						HX_STACK_LINE(71)
						this->sendNotification(HX_CSTRING("changePage"),actionArray->__get((int)1),null());
					}
					else if (  ( _switch_3==HX_CSTRING("pageForward"))){
						HX_STACK_LINE(73)
						this->sendNotification(HX_CSTRING("changePage"),HX_CSTRING("pageForward"),null());
					}
					else if (  ( _switch_3==HX_CSTRING("pageBackward"))){
						HX_STACK_LINE(75)
						this->sendNotification(HX_CSTRING("changePage"),HX_CSTRING("pageBackward"),null());
					}
					else if (  ( _switch_3==HX_CSTRING("playSound"))){
						HX_STACK_LINE(77)
						this->sendNotification(HX_CSTRING("playSound"),actionArray->__get((int)1),null());
					}
				}
			}
		}
...
{
		HX_STACK_PUSH("ActorComponentMediator::addListeners","ca/confidant/glory/view/ActorComponentMediator.hx",33);
		HX_STACK_THIS(this);
		HX_STACK_LINE(36)
		this->actor()->addEventListener(::native::events::MouseEvent_obj::CLICK,this->onActorClicked_dyn(),null(),null(),null());
		HX_STACK_LINE(38)
		this->actor()->set_mouseEnabled(true);
		HX_STACK_LINE(39)
