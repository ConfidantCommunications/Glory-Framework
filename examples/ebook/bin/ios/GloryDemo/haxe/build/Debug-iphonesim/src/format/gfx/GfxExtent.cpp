...
Void GfxExtent_obj::addExtent( Float inX,Float inY){
{
		...
		if (((this->extent == null()))){
			HX_STACK_LINE(20)
			this->extent = ::native::geom::Rectangle_obj::__new(inX,inY,(int)0,(int)0);
			HX_STACK_LINE(21)
			return null();
		}
		HX_STACK_LINE(23)
		if (((inX < this->extent->get_left()))){
			HX_STACK_LINE(23)
			this->extent->set_left(inX);
		}
		HX_STACK_LINE(24)
		if (((inX > this->extent->get_right()))){
			HX_STACK_LINE(24)
			this->extent->set_right(inX);
		}
		HX_STACK_LINE(25)
		if (((inY < this->extent->get_top()))){
			HX_STACK_LINE(25)
			this->extent->set_top(inY);
		}
		HX_STACK_LINE(26)
		if (((inY > this->extent->get_bottom()))){
			HX_STACK_LINE(26)
...
Dynamic GfxExtent_obj::__SetField(const ::String &inName,const Dynamic &inValue,bool inCallProp)
{
	switch(inName.length) {
	case 6:
		if (HX_FIELD_EQ(inName,"extent") ) { extent=inValue.Cast< ::native::geom::Rectangle >(); return inValue; }
	}
