...
Void RemovePageCommand_obj::execute( ::org::puremvc::haxe::interfaces::INotification note){
{
		...
		HX_STACK_LINE(42)
		::native::display::Sprite s = hx::TCast< native::display::Sprite >::cast(pageMediator->getViewComponent());		HX_STACK_VAR(s,"s");
		HX_STACK_LINE(46)
		while(((s->get_numChildren() > (int)0))){
			HX_STACK_LINE(47)
			::native::display::DisplayObject child = s->getChildAt((int)0);		HX_STACK_VAR(child,"child");
			HX_STACK_LINE(48)
			if ((this->facade->hasMediator(child->get_name()))){
				HX_STACK_LINE(48)
				this->facade->removeMediator(child->get_name());
			}
			HX_STACK_LINE(51)
			if ((this->facade->hasProxy(child->get_name()))){
				HX_STACK_LINE(51)
				this->facade->removeProxy(child->get_name());
			}
			HX_STACK_LINE(54)
			s->removeChild(child);
			HX_STACK_LINE(55)
