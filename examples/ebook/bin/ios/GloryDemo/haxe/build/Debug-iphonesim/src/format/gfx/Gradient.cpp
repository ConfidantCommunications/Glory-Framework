...
{
	HX_STACK_LINE(15)
	this->type = ::native::display::GradientType_obj::LINEAR_dyn();
	HX_STACK_LINE(16)
	...
	HX_STACK_LINE(20)
	this->spread = ::native::display::SpreadMethod_obj::PAD_dyn();
	HX_STACK_LINE(21)
	this->interp = ::native::display::InterpolationMethod_obj::RGB_dyn();
	HX_STACK_LINE(22)
	this->focus = 0.0;
}
;
...
Dynamic Gradient_obj::__SetField(const ::String &inName,const Dynamic &inValue,bool inCallProp)
{
	switch(inName.length) {
	case 4:
		if (HX_FIELD_EQ(inName,"type") ) { type=inValue.Cast< ::native::display::GradientType >(); return inValue; }
		break;
	case 5:
		if (HX_FIELD_EQ(inName,"focus") ) { focus=inValue.Cast< Float >(); return inValue; }
		break;
	case 6:
		if (HX_FIELD_EQ(inName,"interp") ) { interp=inValue.Cast< ::native::display::InterpolationMethod >(); return inValue; }
