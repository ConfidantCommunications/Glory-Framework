...
::ca::confidant::glory::view::components::ActorComponent BuildPageCommand_obj::makeActor( ::haxe::xml::Fast actor){
	HX_STACK_PUSH("BuildPageCommand::makeActor","ca/confidant/glory/controller/BuildPageCommand.hx",96);
	HX_STACK_THIS(this);
	...
	HX_STACK_LINE(133)
	a->set_name(actor->att->resolve(HX_CSTRING("id")));
	HX_STACK_LINE(134)
...
Void BuildPageCommand_obj::execute( ::org::puremvc::haxe::interfaces::INotification note){
{
		...
		HX_STACK_LINE(81)
		Array< ::native::display::DisplayObject > controls = crp->getControls();		HX_STACK_VAR(controls,"controls");
		HX_STACK_LINE(82)
		{
			HX_STACK_LINE(82)
			...
			while(((_g < controls->length))){
				HX_STACK_LINE(82)
				::native::display::DisplayObject thisControl = controls->__get(_g);		HX_STACK_VAR(thisControl,"thisControl");
				HX_STACK_LINE(82)
				++(_g);
				HX_STACK_LINE(83)
				this->appMediator->getApp()->removeChild(thisControl);
				HX_STACK_LINE(84)
				this->appMediator->getApp()->addChild(thisControl);
			}
		}
		HX_STACK_LINE(92)
		this->sendNotification(HX_CSTRING("initPageSounds"),pageId,null());
		HX_STACK_LINE(94)
		s->init();
	}
return null();
