...
bool inConvertCubics = __o_inConvertCubics.Default(false);
{
	...
	HX_STACK_LINE(65)
	this->loadGroup(hx::ObjectPtr<OBJ_>(this),svg,::neash::geom::Matrix_obj::__new(null(),null(),null(),null(),null(),null()),null());
}
;
	return null();
}
...
::format::svg::Text SVGData_obj::loadText( ::Xml inText,::neash::geom::Matrix matrix,::Hash inStyles){
...
::format::svg::Path SVGData_obj::loadPath( ::Xml inPath,::neash::geom::Matrix matrix,::Hash inStyles,bool inIsRect,bool inIsEllipse){
	...
	HX_STACK_LINE(506)
	path->stroke_caps = ::neash::display::CapsStyle_obj::ROUND_dyn();
	HX_STACK_LINE(507)
	path->joint_style = ::neash::display::JointStyle_obj::ROUND_dyn();
	HX_STACK_LINE(508)
...
::format::svg::Group SVGData_obj::loadGroup( ::format::svg::Group g,::Xml inG,::neash::geom::Matrix matrix,::Hash inStyles){
...
Void SVGData_obj::loadGradient( ::Xml inGrad,::neash::display::GradientType inType,bool inCrossLink){
{
			...
			for(::cpp::FastIterator_obj< ::Xml > *__it = ::cpp::CreateFastIterator< ::Xml >(inXML->elements());  __it->hasNext(); ){
				::Xml def = __it->next();
				{
					...
					if (((name == HX_CSTRING("linearGradient")))){
						HX_STACK_LINE(326)
						this->loadGradient(def,::neash::display::GradientType_obj::LINEAR_dyn(),(pass == (int)1));
					}
					else{
						HX_STACK_LINE(330)
						if (((name == HX_CSTRING("radialGradient")))){
							HX_STACK_LINE(330)
							this->loadGradient(def,::neash::display::GradientType_obj::RADIAL_dyn(),(pass == (int)1));
						}
					}
				}
;
...
Float SVGData_obj::applyTransform( ::neash::geom::Matrix ioMatrix,::String inTrans){
